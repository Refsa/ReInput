            ...
            if (activeControllerType == ControllerType.KBM)
            {
                ...
                switch (mouseAxis)
                {
                    case MouseAxis.Vertical:
                        onGetAxis?.Invoke(Input.GetAxis("Mouse Y"));
                        // onGetAxis?.Invoke(Mouse.current.delta.ReadValue().y);
                        break;
                    case MouseAxis.Horizontal:
                        onGetAxis?.Invoke(Input.GetAxis("Mouse X"));
                        // onGetAxis?.Invoke(Mouse.current.delta.ReadValue().x);
                        break;
                }
            }
        ...
        bool GetButtonUp()
        {
            ...
            else
            {
                ...
                else if (activeControllerType == ControllerType.Xbox)
                {
                    ...
                    {
                        if (Input.GetKeyUp(xboxButtonKeycode))
                            wasUp = true;
                    }
                }
